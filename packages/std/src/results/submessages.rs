...
mod tests {
    ...
    fn contract_result_deserialization_works() {
        let result: SubMsgResult<u64> = from_slice(br#"{"ok":12}"#).unwrap();
        assert_eq!(result, SubMsgResult::Ok(12));

        let result: SubMsgResult<String> = from_slice(br#"{"ok":"foo"}"#).unwrap();
        assert_eq!(result, SubMsgResult::Ok("foo".to_string()));

        let result: SubMsgResult<Response> =
            from_slice(br#"{"ok":{"messages":[],"attributes":[],"events":[],"data":null}}"#)
                .unwrap();
        assert_eq!(result, SubMsgResult::Ok(Response::default()));

        let result: SubMsgResult<Response> = from_slice(br#"{"error":"broken"}"#).unwrap();
        assert_eq!(result, SubMsgResult::Err("broken".to_string()));

        // ignores whitespace
        let result: SubMsgResult<u64> = from_slice(b" {\n\t  \"ok\": 5898\n}  ").unwrap();
        assert_eq!(result, SubMsgResult::Ok(5898));

        // fails for additional attributes
        let parse: StdResult<SubMsgResult<u64>> = from_slice(br#"{"unrelated":321,"ok":4554}"#);
        match parse.unwrap_err() {
            StdError::ParseErr { .. } => {}
            err => panic!("Unexpected error: {:?}", err),
        }
        let parse: StdResult<SubMsgResult<u64>> = from_slice(br#"{"ok":4554,"unrelated":321}"#);
        match parse.unwrap_err() {
            StdError::ParseErr { .. } => {}
            err => panic!("Unexpected error: {:?}", err),
        }
        let parse: StdResult<SubMsgResult<u64>> =
